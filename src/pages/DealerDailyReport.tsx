            ...
            <table className="w-full text-sm whitespace-nowrap min-w-max">
              <thead>
                <tr className="bg-slate-50/80 border-b border-slate-200">
                  <th className="px-2.5 py-2.5 text-left text-xs font-semibold text-slate-500 sticky left-0 bg-slate-50/80 z-[1]">大区</th>
                  <th className="px-2.5 py-2.5 text-left text-xs font-semibold text-slate-500 sticky left-[48px] bg-slate-50/80 z-[1]">战区</th>
                  {cols.map(c => (
                    <th key={c.key}
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  <tr key={`${row.dealer_id}-${i}`} className="hover:bg-slate-50/70 transition-colors group">
                    ...
                    {cols.map(c => (
                      <td key={c.key} className={`px-2.5 py-2 text-right text-slate-700 ${c.w || ''}`}>{c.fmt((row as any)[c.key])}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
